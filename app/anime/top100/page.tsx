...
import { getTopRatedAnime } from '../../lib/api';
...
function TopRatedAnimePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const data = await getTopRatedAnime(page, 20);
        
        if (data && data.media) {
          setTopAnime(prev => page === 1 ? data.media : [...prev, ...data.media]);
          setHasNextPage(data.pageInfo?.hasNextPage || false);
        }
        setIsLoading(false);
      } catch (err) {
        console.error('Top100動漫數據獲取錯誤:', err);
        setError('無法獲取評分最高的動漫數據，請稍後再試');
        setIsLoading(false);
      }
    };
    ...
    <>
      ...
      <section className="mb-8">
        <div className="relative py-16 px-4 md:px-8 rounded-3xl overflow-hidden mb-10">
          {/* 背景漸變效果 */}
          <div className="absolute inset-0 bg-gradient-to-r from-blue-900 via-blue-700 to-blue-900 opacity-90"></div>
          ...
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-full opacity-10">
              <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                <defs>
                  <radialGradient id="stars-gradient" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
                    <stop offset="0%" stopColor="white" stopOpacity="0.3" />
                    <stop offset="100%" stopColor="white" stopOpacity="0" />
                  </radialGradient>
                </defs>
                <rect x="0" y="0" width="100%" height="100%" fill="url(#stars-gradient)">
          ...
          <div className="relative z-10 text-center">
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-white tracking-wider">TOP 100 ANIME</h1>
            </motion.div>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            <Suspense key={anime.id} fallback={<div className="h-96 bg-gray-800 animate-pulse rounded-2xl"></div>}>
              ...
              >
                <div className="absolute -top-2 -right-2 z-10 bg-white text-black font-bold px-3 py-1 rounded-xl text-xs flex items-center shadow-lg">
                  ...
                  {(anime.meanScore || anime.averageScore) ? (anime.meanScore || anime.averageScore) / 10 : '-'} 分
                </div>
                <div className="absolute -top-2 -left-2 z-10 bg-blue-600 text-white font-bold px-3 py-1 rounded-xl text-xs flex items-center shadow-lg">
                  TOP {page === 1 ? index + 1 : (page - 1) * 20 + index + 1}
                </div>
                <AnimeCard
                  id={anime.id}
                  title={anime.title}
